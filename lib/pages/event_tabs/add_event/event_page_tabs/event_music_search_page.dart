...
class EventMusicSearchPageState extends State<EventMusicSearchPage> {
  ...
  querySpotify(String query) async {
    ...
    query = "https://api.spotify.com/v1/search?q=" +
        query.replaceAll(new RegExp("\s+"), "%20") +
        "&type=track";

    var response = await http.get(Uri.encodeFull(query),
        headers: {"Authorization": "Bearer " + widget._accessToken});
    this.setState(() {
        ...
        child: new Column(
          children: <Widget>[
            ...
            new TextField(
              ...
              decoration:
                  new InputDecoration(hintText: "Search for song to add"),
            ),
