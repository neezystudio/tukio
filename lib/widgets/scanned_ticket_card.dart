...
class ScannedTicketCard extends StatelessWidget {
  ...
  final Map<TicketStatus, String> errorCardDescriptions =
      <TicketStatus, String>{
    ...
    TicketStatus.UnknownUser:
        "The scanned ticket does not belong to a guest going to this event",
    TicketStatus.InvalidEntryCode:
        "The scanned ticket belongs to a guest going to this event, but the security information was not valid",
    TicketStatus.UnableToAccessCamera:
        "Ensure that the camera permissions have been enabled",
  ...
  Widget build(BuildContext context) {
    ...
    if (ticketScan["TicketStatus"] == TicketStatus.Verified) {
      ...
      ticketVerified = true;
    } else if (ticketScan["TicketStatus"] == TicketStatus.AlreadyScanned ||
        ticketScan["TicketStatus"] == TicketStatus.InvalidEntryCode) {
      cardTitle = ticketScan["Name"];
    ...
    if (ticketScan["TicketStatus"] == TicketStatus.UnableToAccessCamera ||
        ticketScan["TicketStatus"] == TicketStatus.ExitedScan ||
        ticketScan["TicketStatus"] == TicketStatus.ScanFailed) {
      scanError = true;
    }
